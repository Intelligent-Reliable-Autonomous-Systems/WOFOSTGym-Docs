...
As all parameters can be specified by command line argument, (see :ref:`crop_soil_agro_command_line`), it is easy to lose track of how data was generated,
...
.. _crop_soil_agro_command_line: 

Crop, Soil, and Agromangement Config
------------------------------------

While all necessary simulation configuration can be done through the ``agromanagement``, ``soil`` and ``crop`` YAML files, 
...
Every argument in the :ref:`agro_template`, :ref:`soil_template`, and :ref:`crop_template`
...
Likewise, we can change ``soil`` and ``crop`` parameters. Suppose we want to change the ``soil`` parameter ``CRAIRC`` to 0.20 which controls the critical soil air content for aeraition, and the 
...
.. important:: 
    ...
    meanwhile, all ``soil`` and ``crop`` arguments are modified by:
